...
const defaultColors: string[] = ['#60a5fa', '#a855f7', '#38bdf8', '#c084fc'];
...
const fragment = /* glsl */ `
  ...
  void main() {
    ...
    if(uAlphaParticles < 0.5) {
      ...
      float circle = smoothstep(0.5, 0.2, d) * 0.9;
...
export default function Particles({
  ...
  useEffect(() => {
    ...
    try {
      renderer = new Renderer({ dpr: Math.min(window.devicePixelRatio || 1, 2), depth: false, alpha: true });
    } catch {
      return;
    }
    ...
    const update = (t: number) => {
      ...
      if (!disableRotation) {
        ...
        particles.rotation.y = Math.cos(elapsed * 0.0004) * 0.12;
